...
# -------- Serial config --------
PRINTER_PORT = "COM7"    # Octopus board
ARDUINO_PORT = "COM3"    # Arduino Mega with pressure sensors
BAUD_PRINTER = 115200
BAUD_ARDUINO = 9600
...
with open("motion_log.csv", mode="w", newline="") as file:
    ...
    writer.writerow(["Time (s)", "Pressure 1 (kPa)", "Pressure 2 (kPa)"])
                ...
                if now >= next_sensor_time:
                    ...
                    if arduino.in_waiting:
                        try:
                            ...
                            millis, p1, p2 = line.split(',')
                            pressure1 = float(p1)
                            pressure2 = float(p2)
                        except:
                            pressure1 = pressure2 = None

                    writer.writerow([round(t_now, 3), pressure1, pressure2])
                    print(f"Time: {round(t_now, 3)} s")
